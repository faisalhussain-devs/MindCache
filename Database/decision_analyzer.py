...
class DecisionStateAnalyzer:
    ...
    def analyze(self, session: Session, topic_id: int):
        """
        ...
        """
        # 1. Fetch all decisions for this topic
        decisions = (
            session.query(DecisionMemory)
            .filter(DecisionMemory.topic_id == topic_id)
        ...
        if new_timestamps:
            earliest = min(new_timestamps) - timedelta(hours=SUPPORT_WINDOW_HOURS)
            latest = max(new_timestamps) + timedelta(hours=SUPPORT_WINDOW_HOURS)
        else:
            return

        # 3. Fetch supporting context within the time window
        ...
        if not raw_response:
            print(f"[DecisionAnalyzer] LLM returned empty for topic {topic_id}")
            return
        ...
        try:
            ...
            if not isinstance(updates, list):
                print(f"[DecisionAnalyzer] Expected list, got {type(updates)}")
                return

        except Exception as e:
            print(f"[DecisionAnalyzer] Parse error: {e}")
            return
        ...
        print(f"[DecisionAnalyzer] Updated {len(updates)} decisions for topic {topic_id}")
