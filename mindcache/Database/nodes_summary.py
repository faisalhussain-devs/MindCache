...
class RecursiveSummarizer:
    ...
    def get_leaf_summary(self, session, topic_id, user_id, min_timestamp=0):
        """
    ...
    def process_leaf(self, session, node, total_mems, user_id):
        ...
        and needs an LLM summary; otherwise returns None.
        """
        ...
        new_data = self.get_leaf_summary(session, node.id, user_id=user_id, min_timestamp=min_timestamp)
        if not new_data:
            return None
            ...
            for ts_key, mem_dict in new_memories.items():
                ...
                for mid, content in mem_dict.items():
                    ...
                    if mid_str not in existing_summary["memories"][ts_key]:
                        ...
                        if ts_key not in delta_memories:
                            delta_memories[ts_key] = {}
            ...
            for ts_key, dec_dict in new_decisions.items():
                ...
                for did, content in dec_dict.items():
                    ...
                    if did_str not in existing_summary["decisions"][ts_key] or existing_summary["decisions"][ts_key][did_str] != content:
                        ...
                        if ts_key not in delta_decisions:
                            delta_decisions[ts_key] = {}
    ...
    def process_leaves_batched(self, session, leaf_nodes, user_id="default"):
        ...
        for node in leaf_nodes:
            ...
            if not existing_desc:
                to_create.append({"node": node, "raw_desc": res["full_raw"]})
                ...
                to_create.append({"node": node, "raw_desc": res["full_raw"], "rebuild": True})

            else:
                to_update.append({
                    "node": node,
                    "raw_desc": res["delta_raw"],
    ...
    def process_parent(self, session, node, user_id, max_depth=None):
        """
                ...
                if not child.children:
                    ...
                    if child.description and len(child.description.strip()) > 16:
                        ...
                        try:
                            ...
                            child_content = str(child.summary or "")
                else:
                    child_content = child.description or ""
